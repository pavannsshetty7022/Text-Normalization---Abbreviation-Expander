const modeGrammarBtn = document.getElementById('mode-grammar');
const grammarSection = document.getElementById('grammar-section');
const grammarInput = document.getElementById('grammar-input');
const grammarSectionBtn = document.getElementById('grammar-section-btn');
const grammarOutput = document.getElementById('grammar-output');
const clearGrammarInputBtn = document.getElementById('clear-grammar-input-btn');
const copyGrammarOutputBtn = document.getElementById('copy-grammar-output-btn');
const clearPlagiarismInputBtn = document.getElementById('clear-plagiarism-input-btn');
const copyPlagiarismOutputBtn = document.getElementById('copy-plagiarism-output-btn');
...
const togglePlagiarismSectionBtn = document.getElementById('toggle-plagiarism-section');
...
const plagiarismSectionBtn = document.getElementById('plagiarism-section-btn');
const plagiarismOutput = document.getElementById('plagiarism-output');
...
const modePlagiarismBtn = document.getElementById('mode-plagiarism');
...
async function handleConversion() {
    ...
    if (convertBtn) convertBtn.classList.add('opacity-50', 'cursor-not-allowed');
    if (convertButtonText) convertButtonText.textContent = 'Converting...';
    ...
    try {
        ...
        if (convertBtn) convertBtn.classList.remove('opacity-50', 'cursor-not-allowed');
        if (convertButtonText) convertButtonText.textContent = 'Convert';
    }
}
...
function handleClearInput() {
    ...
    if (grammarOutput) grammarOutput.value = '';
    if (plagiarismOutput) plagiarismOutput.value = '';
    inputText.focus();
}
...
function updateModeButtons() {
    ...
    if (modeGrammarBtn) modeGrammarBtn.classList.remove('active');
    if (modePlagiarismBtn) modePlagiarismBtn.classList.remove('active');
    if (grammarSection) grammarSection.classList.add('hidden');
    if (plagiarismSection) plagiarismSection.classList.add('hidden');
    document.getElementById('input-text').parentElement.classList.add('hidden');
    document.getElementById('output-text').parentElement.classList.add('hidden');
    document.getElementById('convert-btn').classList.add('hidden');
    document.getElementById('grammar-check-btn').classList.add('hidden');
    document.getElementById('plagiarism-check-btn').classList.add('hidden');
    ...
    if (currentMode === 'sms-to-full') {
        ...
        document.getElementById('input-text').parentElement.classList.remove('hidden');
        document.getElementById('output-text').parentElement.classList.remove('hidden');
        document.getElementById('convert-btn').classList.remove('hidden');
        document.getElementById('grammar-check-btn').classList.add('hidden');
        document.getElementById('plagiarism-check-btn').classList.add('hidden');
    } else if (currentMode === 'full-to-sms') {
        ...
        if (modeGrammarBtn) modeGrammarBtn.classList.add('active');
        if (grammarSection) grammarSection.classList.remove('hidden');
    } else if (currentMode === 'plagiarism') {
        if (modePlagiarismBtn) modePlagiarismBtn.classList.add('active');
        if (plagiarismSection) plagiarismSection.classList.remove('hidden');
    }
...
document.addEventListener('DOMContentLoaded', () => {
    if (modeGrammarBtn) {
        modeGrammarBtn.addEventListener('click', () => {
            currentMode = 'grammar';
            updateModeButtons();
        });
    }

    if (clearGrammarInputBtn && grammarInput) {
        clearGrammarInputBtn.addEventListener('click', () => {
            grammarInput.value = '';
            grammarInput.focus();
        });
    }

    if (copyGrammarOutputBtn && grammarOutput) {
        copyGrammarOutputBtn.addEventListener('click', () => {
            const textToCopy = grammarOutput.value;
            if (textToCopy && textToCopy !== 'Processing...' && !textToCopy.startsWith('Error')) {
                navigator.clipboard.writeText(textToCopy).then(() => {
                    alert("Copied to clipboard!");
                }).catch(err => {
    ...
    if (grammarSectionBtn && grammarInput && grammarOutput) {
        grammarSectionBtn.addEventListener('click', async () => {
            const input = grammarInput.value.trim();
            if (!input) {
                alert("Please enter text to check grammar.");
                return;
            }
            grammarSectionBtn.disabled = true;
            grammarSectionBtn.textContent = 'Checking...';
            grammarOutput.value = "Processing...";
            try {
                ...
                const data = await response.json();
                grammarOutput.value = data.feedback ? data.feedback.join('\n') : "No grammar or spelling errors found.";
            } catch (error) {
                grammarOutput.value = "Error checking grammar. Please try again.";
            } finally {
                grammarSectionBtn.disabled = false;
                grammarSectionBtn.textContent = 'Check Grammar';
            }
        });
    }
    if (clearPlagiarismInputBtn && plagiarismInput) {
        clearPlagiarismInputBtn.addEventListener('click', () => {
            plagiarismInput.value = '';
            plagiarismInput.focus();
        });
    }
    if (copyPlagiarismOutputBtn && plagiarismOutput) {
        copyPlagiarismOutputBtn.addEventListener('click', () => {
            const textToCopy = plagiarismOutput.value;
            if (textToCopy && textToCopy !== 'Processing...' && !textToCopy.startsWith('Error')) {
                navigator.clipboard.writeText(textToCopy).then(() => {
                    alert("Copied to clipboard!");
                }).catch(err => {
    ...
    if (modePlagiarismBtn) {
        modePlagiarismBtn.addEventListener('click', () => {
    ...
    if (togglePlagiarismSectionBtn && plagiarismSection) {
        togglePlagiarismSectionBtn.addEventListener('click', () => {
            plagiarismSection.classList.toggle('hidden');
        });
    }

    if (plagiarismSectionBtn && plagiarismInput && plagiarismOutput) {
        plagiarismSectionBtn.addEventListener('click', async () => {
            const input = plagiarismInput.value.trim();
            if (!input) {
                alert("Please enter text to check for plagiarism.");
                return;
            }
            plagiarismSectionBtn.disabled = true;
            plagiarismSectionBtn.textContent = 'Checking...';
            plagiarismOutput.value = "Processing...";
            try {
                const response = await fetch('http://127.0.0.1:5000/process_text', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                plagiarismOutput.value = displayText;
            } catch (error) {
                plagiarismOutput.value = "Error checking plagiarism. Please try again.";
            } finally {
                plagiarismSectionBtn.disabled = false;
                plagiarismSectionBtn.textContent = 'Check Plagiarism';
            }
        });
    }
    ...
    if (grammarCheckBtn) grammarCheckBtn.addEventListener('click', handleGrammarCheck);
    if (typeof handlePlagiarismCheck === 'function' && plagiarismCheckBtn) {
    ...
    sidebarFeedbackBtn.addEventListener('click', () => {
        closeSidebar();
        openModal('Give Feedback', `<p class="text-center text-lg py-4">You can provide feedback via email at [shettypavan524@gmail.com](mailto:shettypavan524@gmail.com).</p>`);
    });
